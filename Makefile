...
# Load persisted module config if it exists (written by make update_list)
-include sources/.proj_config
...
.PHONY: help
help:
	@echo "=== $(PROJECT) Build System (Verilog/SystemVerilog) ==="
	@echo ""
...
update_list:
	...
	@find $(RTL_DIR) \( -name "*.v" -o -name "*.sv" \) | sort >> $(FILELIST) 2>/dev/null || true

	@echo "" >> $(FILELIST)
	@echo "# Testbench Files" >> $(FILELIST)
	@find $(TB_DIR) -name "*_tb.v" -o -name "*_tb.sv" -o -name "tb_*.v" -o -name "tb_*.sv" | sort >> $(FILELIST) 2>/dev/null || true

	@echo "File list updated: $(FILELIST)"
	@echo "Found $$(grep -c '^[^#[:space:]]' $(FILELIST) 2>/dev/null || echo 0) files"

	@if [ "$(origin TOP_MODULE)" = "command line" ] || [ "$(origin TESTBENCH)" = "command line" ]; then \
		echo "# Auto-generated by make update_list — do not edit manually" > sources/.proj_config; \
		echo "TOP_MODULE ?= $(TOP_MODULE)" >> sources/.proj_config; \
		echo "TESTBENCH  ?= $(TESTBENCH)"  >> sources/.proj_config; \
		echo "Config saved: TOP_MODULE=$(TOP_MODULE), TESTBENCH=$(TESTBENCH)"; \
	fi
